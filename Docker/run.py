...
user = '****'         # 你的学号
password = '****'      # 你的密码
location = '广东省广州市番禺区小谷围街道华南师范大学大学城校区'  # 详细位置(xx省xx市xx区/县)
'''
# travel = 'xx省xx市'
# [可选]健康码、行程卡
health_code = None
...
def clockin(id,par):
    ...
    user = id # 你的学号
    password = par[0]  # 你的密码
    location = par[1] # 详细位置(xx省xx市xx区/县)
    ...
    try:
        ...
        if res2.text.find("alert('打卡成功')") == -1:
            # print('打卡失败', '请查看打卡内容是否变更')
            return ('打卡失败', '请查看打卡内容是否变更')
        
        # print('打卡成功','今天打卡额温为' + str(tem) + '°C')
        return ('打卡成功','今天打卡额温为' + str(tem) + '°C')
        
    except Exception as e:
        # print('打卡失败', '错误信息: ' + str(e))
        return ('打卡失败', '错误信息: ' + str(e))

# ====================
students = {}

with open('/mnt/stu_id.txt',mode='r',encoding="utf-8") as f:
    for line in f:
        stu = line.split(' ')
        stu = [x.strip() for x in stu]
        students[stu[0]] = stu[1],stu[2]

print(students)

for id in students:
    print(id)
    clockin(id,students[id])
